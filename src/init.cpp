...
void PrepareShutdown()
{
    ...
    util::ThreadRename("ibilechain-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all IBLC specific functionality (Masternodes) (0-1, default: %u)"), 0));
...
{
    util::ThreadRename("ibilechain-loadblk");
...
/** Sanity checks
 *  Ensure that Ibilechain is running in a usable environment with all
...
#endif
    LogPrintf("Ibilechain version %s (%s)\n", version_string, CLIENT_DATE);
}

/** Initialize ibilechain.
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. Ibilechain is shutting down."));

    std::string strDataDir = GetDataDir().string();

    // Make sure only a single Ibilechain process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. Ibilechain is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //Ibilechain specific: spork DB's
                ...
                // Ibilechain: load previous sessions sporks if we have them.
