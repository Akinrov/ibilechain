...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter IBLC address (e.g. %1)").arg("iQeVBSRsUWWHQnquWKJSvZPttbXrWQRtoT"));
...
{
    // return if URI is not valid or is no IBLC: URI
...
{
    // Convert ibilechain:// to ibilechain:
    //
    //    Cannot handle this later, because ibilechain:// will cause Qt to see the part after // as host,
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ibilechain.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for ibilechain.lnk
...
{
    return GetAutostartDir() / "ibilechain.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a ibilechain.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Ibilechain\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the ibilechain app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add ibilechain app to startup item list
