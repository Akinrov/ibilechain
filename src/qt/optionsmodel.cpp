...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    ...
    if (settings.contains("nAnonymizeIbilechainAmount") || reset)
        SoftSetArg("-anonymizeibilechainamount", settings.value("nAnonymizeIbilechainAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ibilechain.cpp during shotdown to also remove the window positions
