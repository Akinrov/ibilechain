...
#include "qt/ibilechain/defaultdialog.h"
#include "qt/ibilechain/settings/settingsfaqwidget.h"
...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Ibilechain client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Ibilechain"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Ibilechain"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
