...
const char * const PIVX_CONF_FILENAME = "ibilechain.conf";
const char * const PIVX_PID_FILENAME = "ibilechain.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";
const char * const PIVX_ACTIVE_MASTERNODE_CONF_FILENAME = "activemasternode.conf";


// Ibilechain only features
// Masternode
...
#else
    const char* pszModule = "ibilechain";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\ibilechain
// Windows >= Vista: C:\Users\Username\AppData\Roaming\ibilechain
// Mac: ~/Library/Application Support/ibilechain
// Unix: ~/.ibilechain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ibilechain";
#else
    fs::path pathRet;
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty ibilechain.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override ibilechain.conf
