...
CAmount CMasternode::GetMasternodeNodeCollateral(int nHeight) 
{
    ...
    // if(nHeight >   2000) return     50 * COIN;
    if(nHeight >    50) return     25000 * COIN;
...
CAmount CMasternode::GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 574010 || nHeight == 585330 || nHeight == 586594) return 4 * COIN;

    if(nHeight > 11935500) return    0 * COIN;
    if(nHeight >       1) return      4 * COIN;
    if(nHeight >       0) return 2258000 * COIN;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 IBLC tx got MASTERNODE_MIN_CONFIRMATIONS
    ...
    if (mi != mapBlockIndex.end() && (*mi).second) {
        ...
        int nConfHeight = pMNIndex->nHeight + MASTERNODE_MIN_CONFIRMATIONS - 1; // block for 1000 IBLC tx -> 1 confirmation
