...
- [Building PIVX Core](#building-ibilechain-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/ibilechain-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/ibilechain-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/ibilechain-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/ibilechain-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/ibilechain-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/ibilechain-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/ibilechain-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@ibilechain.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/ibilechain-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/ibilechain-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/ibilechain-osx-*-build.assert
```
