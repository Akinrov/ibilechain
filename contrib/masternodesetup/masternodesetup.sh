...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='ibilechain.conf'
CONFIGFOLDER='/root/.ibilechain'
COIN_DAEMON='ibilechaind'
COIN_CLI='ibilechain-cli'
COIN_PATH='/usr/local/bin/'
COIN_TGZ=`curl -s https://api.github.com/repos/ibilechain/IBLC/releases/latest | grep "browser_download_url.*Linux\\.zip" | cut -d : -f 2,3 | tr -d \" | xargs`
COIN_ZIP=$(echo $COIN_TGZ | awk -F'/' '{print $NF}')
COIN_NAME='ibilechain'
COIN_PORT=3424
RPC_PORT=3423
